...
export default class extends BaseApi {
  getAll = async () => {
    try {
      let urlBase = this.makeUrl("categories");
  ...
  getById = async (id = "") => {
    try {
      let urlBase = this.makeUrl("categories", id.toString());
      let result = await this.execute_get(urlBase);
      return result;
    } catch (error) {
      console.log(error);
  ...
  update = async (id, model) => {
    try {
      let urlBase = this.makeUrl("categories", id);
  ...
  deleteMany = async (model) => {
    try {
      let urlBase = this.makeUrl("categories", 'delete-many');
      let result = await this.execute_post(urlBase, model);
      return result;
    } catch (error) {
      console.log(error);
